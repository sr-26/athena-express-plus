...
module.exports = class AthenaExpress {
    constructor(init) {
        ...
        this.config = {
            ...
            resultReuse: init.resultReuse || false,
            resultReuseMaxAge: Number(init.resultReuseMaxAge) || 0
        };
    }

    async query(query, values, options) {
        const config = { ...this.config, ...(options && options)};
        ...
        if (typeof query === "object") {
            ...
            if (loweredCaseKeys.hasOwnProperty("values")) {
                config.values = loweredCaseKeys.values;
            }
