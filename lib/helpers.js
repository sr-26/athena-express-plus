...
function startQueryExecution(config) {
  ...
  if (config.resultReuse) {
    params.ResultReuseConfiguration = {
      ResultReuseByAgeConfiguration: {
        Enabled: true
      }
    }
    if (config.hasOwnProperty('resultReuseMaxAge') && config.resultReuseMaxAge >= 0 && config.resultReuseMaxAge <= 10080) {
      params.ResultReuseConfiguration.ResultReuseByAgeConfiguration.MaxAgeInMinutes = config.resultReuseMaxAge;
    }
  }
